...
import numpy as np
import mlflow
from models import ECG_CNN_VAE_Encoder, ECG_Classifier, ECG_VAE
from utils import *
...
def main_train_test(pretrain_signals, pre_train_labels, data_df, embedded_size=256, kernel_size=15, dropout=0.3, pretrain_num_epochs=100, num_epochs=100, print_results = False, save_results = False, plot_results = False, seed=42, domain ='time' , exp_name=None, encoder_name='auto_encoder', pre_batch_size=128, batch_size=128, pretrain_encoder_path=None, pretrain=True):
    ...
    encoder = ECG_CNN_VAE_Encoder(signal_length=signal_length, embedded_size=embedded_size, kernel_size=kernel_size, dropout=dropout, seed=seed).to(device)
    decoder = ECG_Decoder(encoded_size=embedded_size, output_channels=12, target_length=signal_length, seed=seed).to(device)
    vae = ECG_VAE(encoder=encoder, decoder=decoder).to(device)
    ...
    if pretrain: 
        ...
        if os.path.exists(encoder_path):
            ...
            pretrain_optimizer = optim.Adam(vae.parameters(), lr=pretrain_lr, weight_decay=1e-5)
            ...
            for epoch in tqdm(range(pretrain_num_epochs)):
                ...
                for signals, _ in pretrain_train_loader:
                    ...
                    reconstructed_signal, mu, logvar = vae(signals)
                    tr_reconst_loss, _, _ = vae_loss_function(reconstructed_signal, signals, mu, logvar)
                ...
                with torch.no_grad():
                    for signals, _ in pretrain_val_loader:
                        ...
                        reconstructed_signal, mu, logvar = vae(signals)
                        vl_reconst_loss, _, _ =  vae_loss_function(reconstructed_signal, signals, mu, logvar)
    ...
    classifier = ECG_Classifier(vae.encoder, embedded_size=embedded_size, dropout=dropout).to(device)
